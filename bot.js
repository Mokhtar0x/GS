...
// Create bot with Termux-friendly polling settings
const bot = new TelegramBot(config.BOT_TOKEN, {
    polling: {
        ...
        params: {
            ...
            limit: config.TERMUX_MODE ? 5 : 10,
            allowed_updates: ['message', 'callback_query']
        }
    },
    request: config.TERMUX_MODE ? {
...
// Forced subscription settings
const FORCE_SUBSCRIPTION_ENABLED = String(
    process.env.FORCE_SUBSCRIPTION ??
    config.FORCE_SUBSCRIPTION ??
    'true'
).toLowerCase() === 'true';

const FORCE_SUB_CHANNEL =
    process.env.FORCE_SUB_CHANNEL ||
...
const FORCE_SUB_CHANNEL_URL =
    process.env.FORCE_SUB_CHANNEL_URL ||
    process.env.CHANNEL_URL ||
    config.FORCE_SUB_CHANNEL_URL ||
    config.CHANNEL_URL ||
    config.REQUIRED_CHANNEL_URL ||
    '';

const FORCE_SUB_CHANNEL_NAME =
    process.env.FORCE_SUB_CHANNEL_NAME ||
    config.FORCE_SUB_CHANNEL_NAME ||
    config.REQUIRED_CHANNEL_NAME ||
    config.CHANNEL_NAME ||
    FORCE_SUB_CHANNEL ||
    'القناة';
...
bot.on('polling_error', (error) => {
    ...
    if (error.code === 'ETELEGRAM' && error.response?.body?.error_code === 409) {
        ...
        console.log('🛑 Stopping polling to avoid duplicated updates. Make sure only one bot instance is running.');
        bot.stopPolling().catch(() => {});
        return;
    } else if (error.code === 'EFATAL' || error.code === 'ESOCKETTIMEOUT' || error.code === 'ECONNRESET') {
        console.log(`⚠️ Connection error (${error.code}). Waiting for the next polling cycle without forcing a restart.`);
        return;
    }
});
...
const pendingBulkActions = new Map(); // Temp storage for bulk action IDs to avoid callback_data 64-byte limit
const processedMessages = new Map();
const processedCallbacks = new Map();
const DEDUPE_WINDOW_MS = 15000;

function isDuplicate(map, key, windowMs = DEDUPE_WINDOW_MS) {
    const now = Date.now();
...
function isForcedSubscriptionEnabled() {
    return Boolean(
        FORCE_SUBSCRIPTION_ENABLED &&
        FORCE_SUB_CHANNEL &&
        String(FORCE_SUB_CHANNEL).trim()
    );
}

function normalizeChannelUsername(channelValue) {
    ...
    if (value.startsWith('http://t.me/')) return `@${value.replace('http://t.me/', '').replace(/^@+/, '')}`;
    if (value.startsWith('t.me/')) return `@${value.replace('t.me/', '').replace(/^@+/, '')}`;
    if (value.startsWith('@')) return value;
    return `@${value.replace(/^@+/, '')}`;
}

function getForceSubscriptionJoinUrl() {
    if (FORCE_SUB_CHANNEL_URL && String(FORCE_SUB_CHANNEL_URL).trim()) {
        return String(FORCE_SUB_CHANNEL_URL).trim();
    }

    const normalized = normalizeChannelUsername(FORCE_SUB_CHANNEL);
...
function getForceSubscriptionKeyboard() {
    const joinUrl = getForceSubscriptionJoinUrl();
    const inlineKeyboard = [];

    if (joinUrl) {
        inlineKeyboard.push([{ text: '📢 اشترك في القناة', url: joinUrl }]);
    }

    inlineKeyboard.push([{ text: '✅ تحقق من الاشتراك', callback_data: 'verify_subscription' }]);
...
async function sendForceSubscriptionPrompt(chatId) {
    const channelLabel = FORCE_SUB_CHANNEL_NAME || normalizeChannelUsername(FORCE_SUB_CHANNEL) || 'القناة';
    const joinUrl = getForceSubscriptionJoinUrl();
    const subscriptionMessage = joinUrl
        ? `📢 للاستخدام يجب الاشتراك أولاً في القناة الإجبارية:
...
بعد الاشتراك اضغط على زر "تحقق من الاشتراك".`;
    ...
    return bot.sendMessage(chatId, subscriptionMessage, getForceSubscriptionKeyboard());
}

async function checkUserChannelSubscription(userId) {
    if (!isForcedSubscriptionEnabled()) {
        return true;
    }

    try {
        const channelRef = normalizeChannelUsername(FORCE_SUB_CHANNEL) || FORCE_SUB_CHANNEL;
        const member = await bot.getChatMember(channelRef, userId);
        const allowedStatuses = ['creator', 'administrator', 'member'];
        return allowedStatuses.includes(member.status);
    } catch (error) {
        logError(error, `FORCED_SUBSCRIPTION_CHECK user:${userId}`);
        return false;
    }
}


async function safeAnswerCallbackQuery(botInstance, callbackQueryId, options = {}) {
    if (!callbackQueryId) return false;
    try {
        await botInstance.answerCallbackQuery(callbackQueryId, options);
        return true;
    } catch (error) {
        ...
        console.error('answerCallbackQuery error:', desc);
        return false;
    }
}

async function ensureUserSubscribed(chatId, userId, options = {}) {
    const { callbackQueryId = null, showVerifiedMessage = false } = options;

    if (!isForcedSubscriptionEnabled() || isAdmin(userId)) {
        return true;
    }

    const isSubscribed = await checkUserChannelSubscription(userId);

    if (isSubscribed) {
        if (callbackQueryId) {
            await safeAnswerCallbackQuery(bot, callbackQueryId, {
                text: showVerifiedMessage ? '✅ تم التحقق من اشتراكك بنجاح' : '✅ تم التحقق',
                show_alert: false
            }).catch(() => {});
        }
        return true;
    ...
    await sendForceSubscriptionPrompt(chatId);
    return false;
}

// Helper functions

function formatTaskDecisionMessage({ type = 'gmail', status = 'approved', email, reward = null, language = 'ar' }) {
    const isApproved = status === 'approved';
    const accountLabel = type === 'email'
        ? (language === 'en' ? 'Email' : 'الإيميل')
        : (language === 'en' ? 'Gmail' : 'الجيميل');

    if (isApproved) {
        return language === 'en'
            ? `✅ Your ${type === 'email' ? 'email' : 'Gmail'} account has been approved!

💰 Reward added: ${reward || ''}
📧 ${accountLabel}: ${email}`
            : `✅ تم قبول ${type === 'email' ? 'حساب الإيميل' : 'حساب الجيميل'} الخاص بك!

💰 تم إضافة المكافأة: ${reward || ''}
📧 ${accountLabel}: ${email}`;
    }

    return language === 'en'
        ? `❌ Your ${type === 'email' ? 'email' : 'Gmail'} account was rejected.

📧 ${accountLabel}: ${email}

💡 Please review the task instructions and try again.`
        : `❌ تم رفض ${type === 'email' ? 'حساب الإيميل' : 'حساب الجيميل'} الخاص بك.
...
function formatWithdrawalCompletedUserMessage(request, language = 'ar') {
    return language === 'en'
        ? `✅ Withdrawal completed successfully!

💰 Amount: ${formatBalance(request.amount, request.currency)}
💳 Method: ${request.method}
📋 Wallet / Account: ${request.details}
📅 Completed: ${new Date().toLocaleString()}

💡 Please check your wallet.
📋 Request ID: #${request.id || request.request_id || ''}`
        : `✅ تم إكمال السحب بنجاح!

💰 المبلغ: ${formatBalance(request.amount, request.currency)}
💳 الطريقة: ${request.method}
📋 المحفظة / الحساب: ${request.details}
📅 تاريخ الإكمال: ${new Date().toLocaleString()}

💡 تأكد من محفظتك.
📋 رقم الطلب: #${request.id || request.request_id || ''}`;
}
...
async function processReferralReward(userId) {
    try {
        ...
        // Get referral reward amount (fixed amount per task)
        const rewardAmount = parseFloat(await db.getSetting('referral_reward_egp') || config.REFERRAL_REWARD_EGP);
        const currency = referrer.preferred_currency || 'EGP';

        // Add reward to referrer's balance
        if (currency === 'USD') {
            const usdReward = await convertEGPToUSD(rewardAmount);
            const newBalance = (parseFloat(referrer.balance_usd) || 0) + usdReward;
        ...
        // Record this referral reward
        await db.addReferralReward(referrer.id, userId, rewardAmount, currency);

        // Send notification to referrer
        const language = await getUserLanguage(referrer.id);
        const formattedAmount = formatBalance(rewardAmount, currency);
        const referredUsername = user.username || 'مستخدم';
        
        const message = language === 'en' ?
            `🎉 Referral Reward!\n\n💰 You earned ${formattedAmount}\n👤 From: @${referredUsername}\n📱 For completing a Gmail task\n\n✨ Keep inviting friends to earn more!` :
            `🎉 مكافأة إحالة!\n\n💰 حصلت على ${formattedAmount}\n👤 من: @${referredUsername}\n📱 لإكمال مهمة جيميل\n\n✨ استمر في دعوة الأصدقاء لتربح أكثر!`;
        
        await safeSendMessage(referrer.id, message);
        
        console.log(`Referral reward of ${rewardAmount} ${currency} sent to user ${referrer.id} for referral ${userId}`);
    } catch (error) {
...
async function showReferralMenu(chatId, userId, language) {
    try {
        ...
        // Get reward amount
        const rewardEGP = parseFloat(await db.getSetting('referral_reward_egp') || config.REFERRAL_REWARD_EGP);
        const rewardDisplay = formatBalance(rewardEGP, 'EGP');
        
        // Get list of referred users to count active ones
        const referrals = await db.getUserReferrals(userId);
        const uniqueReferrals = new Set(referrals.map(r => r.referred_id)).size;
        
        const message = language === 'en' ?
            `🔗 Referral System\n\n💰 Earn ${rewardDisplay} for EVERY Gmail task your friends complete!\n\n📊 Your Stats:\n👥 Friends Invited: ${uniqueReferrals}\n🎯 Total Tasks: ${stats.completed_referrals}\n💰 Total Earned: ${formatBalance(stats.total_earned, currency)}\n\n💡 Your friends keep working = You keep earning!\n✨ Unlimited earning potential!` :
            `🔗 نظام الإحالة\n\n💰 احصل على ${rewardDisplay} عن كل مهمة جيميل يكملها أصدقاؤك!\n\n📊 إحصائياتك:\n👥 الأصدقاء المدعوين: ${uniqueReferrals}\n🎯 إجمالي المهام: ${stats.completed_referrals}\n💰 إجمالي الأرباح: ${formatBalance(stats.total_earned, currency)}\n\n💡 أصدقاؤك يعملون = أنت تربح!\n✨ إمكانية ربح غير محدودة!`;
...
async function showReferralCode(chatId, userId, language) {
    try {
        ...
        // Get reward amount
        const rewardAmount = formatBalance(parseFloat(await db.getSetting('referral_reward_egp') || config.REFERRAL_REWARD_EGP), 'EGP');
        
        // Create message with clickable link
        const message = language === 'en' ? 
            `🔗 *Your Referral Link:*\n\n\`${telegramLink}\`\n\n📋 Copy this link and share it with your friends\n💰 You will earn ${rewardAmount} for EVERY Gmail task they complete!\n\n🎯 *Your Referral Code:* \`${referralCode}\`\n\n💡 The more tasks they complete, the more you earn!\n✨ Unlimited earning potential!\n\n👆 Click the link above to test it!` :
            `🔗 *رابط الإحالة الخاص بك:*\n\n\`${telegramLink}\`\n\n📋 انسخ هذا الرابط وشاركه مع أصدقائك\n💰 ستحصل على ${rewardAmount} عن كل مهمة جيميل يكملونها!\n\n🎯 *كود الإحالة:* \`${referralCode}\`\n\n💡 كلما أكملوا مهام أكثر، ربحت أكثر!\n✨ إمكانية ربح غير محدودة!\n\n👆 اضغط على الرابط أعلاه لتجربته!`;
...
async function showReferralStats(chatId, userId, language) {
    try {
        ...
        const message = language === 'en' ?
            `📊 Referral Statistics:\n\n👥 Total Invited: ${stats.total_referrals || 0}\n✅ Active Referrals: ${activeReferrals}\n🎯 Total Tasks Completed: ${stats.completed_referrals || 0}\n💰 Total Earnings: ${totalEarned}\n💵 Reward per Task: ${rewardDisplay}\n\n🔗 Referral Code: \`${referralCode}\`\n\n💡 You earn ${rewardDisplay} for EVERY Gmail task your friends complete!\n✨ The more they work, the more you earn!` :
            `📊 إحصائيات الإحالة:\n\n👥 إجمالي المدعوين: ${stats.total_referrals || 0}\n✅ الإحالات النشطة: ${activeReferrals}\n🎯 إجمالي المهام المكتملة: ${stats.completed_referrals || 0}\n💰 إجمالي الأرباح: ${totalEarned}\n💵 المكافأة لكل مهمة: ${rewardDisplay}\n\n🔗 كود الإحالة: \`${referralCode}\`\n\n💡 تحصل على ${rewardDisplay} عن كل مهمة جيميل يكملها أصدقاؤك!\n✨ كلما عملوا أكثر، ربحت أكثر!`;
...
async function showReferralList(chatId, userId, language) {
    try {
        ...
        for (let i = 0; i < Math.min(referrals.length, 20); i++) {
            const ref = referrals[i];
            const username = ref.referred_username || 'Unknown';
            const status = ref.status === 'completed' ? 
                (language === 'en' ? '✅ Completed' : '✅ مكتمل') : 
                (language === 'en' ? '⏳ Pending' : '⏳ معلق');
            const reward = ref.status === 'completed' ? 
                formatBalance(ref.reward_earned, ref.reward_currency) : 
                (language === 'en' ? 'Not earned yet' : 'لم يتم الكسب بعد');
        ...
        if (referrals.length > 20) {
            message += language === 'en' ?
                `\n📊 Showing first 20 of ${referrals.length} referrals` :
                `\n📊 عرض أول 20 من ${referrals.length} إحالة`;
        }
...
bot.onText(/\/start(.*)/, async (msg, match) => {
    ...
    try {
        ...
        if (isAdmin(userId)) {
            ...
            const language = 'ar';
            const isSubscribed = await ensureUserSubscribed(chatId, userId);
            ...
            if (isSubscribed) {
                ...
                await bot.sendMessage(chatId, getMessage('CHOOSE_TASKS', language));
            }
        }
    } catch (error) {
...
bot.on('message', async (msg) => {
    ...
    const userId = msg.from.id.toString();
    const text = msg.text;
    const messageKey = `${chatId}:${msg.message_id}`;

    if (isDuplicate(processedMessages, messageKey)) {
        return;
    }
        ...
        default:
            ...
            if (text.startsWith('📱 مهمة إنشاء جيميل') || text.startsWith('📱 Gmail Creation Task')) {
                ...
                if (gmailTasksEnabled) {
                    ...
                    const message = language === 'en'
  ? "🚧 Notice\nDue to the high demand on the service, Gmail account creation has been temporarily suspended to organize requests and ensure service quality.\n\n🔔 You will be automatically notified once the service resumes.\n🙏 Thank you for your understanding and patience."
...
bot.sendMessage(chatId, message);
        ...
        case '⏳ Pending Balance':
            await showPendingBalance(chatId, userId, language);
            break;
...
async function handleAdminButtons(chatId, userId, text, language) {
    switch (text) {
        ...
        case '➕ إضافة لقائمة التصدير':
        case '➕ Add to Export List':
            userStates.set(userId, 'waiting_add_export_emails');
            bot.sendMessage(
                chatId,
                language === 'en'
                    ? `➕ Send the emails you want to add to the persistent export list, one per line.

Example:
first@gmail.com
second@gmail.com`
                    : `➕ أرسل الإيميلات التي تريد إضافتها لقائمة التصدير الدائمة، كل إيميل في سطر.

مثال:
first@gmail.com
second@gmail.com`,
                keyboards.getKeyboard('cancelAdmin', language)
            );
            break;

        case '➖ حذف من قائمة التصدير':
        case '➖ Remove from Export List':
            userStates.set(userId, 'waiting_remove_export_emails');
            bot.sendMessage(
                chatId,
                language === 'en'
                    ? '➖ Send the emails you want to remove from the persistent export list, one per line.'
                    : '➖ أرسل الإيميلات التي تريد حذفها من قائمة التصدير الدائمة، كل إيميل في سطر.',
                keyboards.getKeyboard('cancelAdmin', language)
            );
            break;
...
// await bot.sendMessage(chatId, warningMessage);
        ...
        const message = language === 'en' ?
            ...
            `📋 قائمة المهام المتاحة\n\n💰 اختر مهمة الآن وابدأ تحقيق الأرباح.\n⚡ كلما أنجزت مهام أكثر زادت أرباحك.\n\n⏳ سيتم إضافة الأرباح إلى حسابك خلال 24 إلى 48 ساعة كحد أقصى.`;
...
// Show pending balance with email details
async function showPendingBalance(chatId, userId, language) {
    try {
        let user = await db.getUser(userId);
        ...
        const allPendingGmails = await db.getPendingGmailAccounts();
        const userPendingGmails = allPendingGmails.filter(gmail => String(gmail.user_id) === String(userId));
        ...
        let pendingAmount = 0;
        if (currency === 'USD') {
            pendingAmount = await convertEGPToUSD(gmailReward * userPendingGmails.length);
        } else {
            pendingAmount = gmailReward * userPendingGmails.length;
        }

        let message = language === 'en' ?
            `⏳ Pending Balance:

💰 Total Pending: ${formatBalance(pendingAmount, currency)}

` :
            `⏳ الأموال المعلقة:

💰 إجمالي المعلق: ${formatBalance(pendingAmount, currency)}

`;

        if (userPendingGmails.length === 0) {
            message += language === 'en' ?
                '📧 No pending emails currently under review.' :
                '📧 لا يوجد إيميلات قيد المراجعة حالياً.';
            return bot.sendMessage(chatId, message);
        }

        message += language === 'en' ?
            `📧 Pending Emails (${userPendingGmails.length}):

` :
            `📧 الإيميلات قيد المراجعة (${userPendingGmails.length}):

`;

        for (let i = 0; i < userPendingGmails.length; i++) {
            const gmail = userPendingGmails[i];
            const date = new Date(gmail.created_at).toLocaleDateString('ar-EG');
            message += language === 'en' ?
                `${i + 1}. ${gmail.email}
⏳ Pending | 📅 ${date}
...
`;
        ...
        console.error('Error showing pending balance:', error);
        const errorMessage = language === 'en' ?
            '❌ Error showing pending balance' :
            '❌ حدث خطأ في عرض الأموال المعلقة';
        bot.sendMessage(chatId, errorMessage);
    }
}
...
async function handleUserState(chatId, userId, text, state, language) {
    ...
    switch (stateValue) {
        ...
        case 'waiting_add_export_emails':
            await processAddExportEmails(chatId, userId, text, language);
            break;

        case 'waiting_remove_export_emails':
            await processRemoveExportEmails(chatId, userId, text, language);
            break;
...
async function processGmailEmail(chatId, userId, text, language) {
    try {
        ...
        const normalizedEmail = db.normalizeEmail(text);
        ...
        const emailExists = await db.checkGmailEmailExists(normalizedEmail);
        if (emailExists) {
            const message = language === 'en' ?
        ...
        await db.addGmailAccount(userId, normalizedEmail);
        await db.removeActiveTask(userId);
        userStates.delete(userId);
...
async function initiateWithdrawal(chatId, userId, language) {
    try {
        ...
        // If user does not exist, create them first
        if (!user) {
            await db.addUser(userId, null);
            user = await db.getUser(userId);
        }
        ...
        if (currentBalance <= 0) {
            const message = language === 'en' ?
                '❌ Your balance is zero. Complete tasks first, then try withdrawing again.' :
                '❌ رصيدك الحالي صفر. أكمل بعض المهام أولاً ثم حاول السحب مرة أخرى.';
            return bot.sendMessage(chatId, message);
        }
...
📊 Minimum withdrawal: ${formatBalance(minWithdrawal, 'EGP')}` :
                `❌ رصيدك غير كافٍ للسحب.

💰 رصيدك الحالي: ${formatBalance(currentBalance, 'EGP')}
📊 الحد الأدنى للسحب: ${formatBalance(minWithdrawal, 'EGP')}`;
            return bot.sendMessage(chatId, message);
        }
...
✍️ أرسل المبلغ الذي تريد سحبه:`;
        ...
        const message = language === 'en' ?
            '❌ Error starting withdrawal request' :
            '❌ حدث خطأ أثناء بدء طلب السحب';
        return bot.sendMessage(chatId, message);
    }
}
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (isDuplicate(processedCallbacks, callbackQuery.id)) {
        return;
    }

    try {
        await safeAnswerCallbackQuery(bot, callbackQuery.id);
        ...
        if (data === 'verify_subscription') {
            ...
            if (isSubscribed) {
                const keyboard = keyboards.getKeyboard('userKeyboard', language);
                await bot.sendMessage(chatId, '✅ تم التحقق من الاشتراك، يمكنك الآن استخدام البوت.', keyboard);
                await bot.sendMessage(chatId, getMessage('CHOOSE_TASKS', language));
            }
            return;
        }
        ...
        const adminOnlyButtons = data.startsWith('approve_gmail_') ||
            data.startsWith('reject_gmail_') ||
            data.startsWith('ban_user_') ||
            data.startsWith('unban_user_') ||
            data.startsWith('edit_balance_') ||
            data.startsWith('message_user_') ||
            data.startsWith('user_details_') ||
            data.startsWith('refresh_user_') ||
            data.startsWith('gmail_page_') ||
            data === 'toggle_email_tasks' ||
            data === 'toggle_gmail_tasks' ||
            data === 'bulk_approve_confirm' ||
            data === 'bulk_approve_cancel' ||
            data === 'bulk_reject_confirm' ||
            data === 'bulk_reject_cancel' ||
            data.startsWith('selective_approve_') ||
            data === 'selective_approve_cancel' ||
            data.startsWith('selective_reject_') ||
            data === 'selective_reject_cancel' ||
            data.startsWith('complete_withdrawal_');

        if (!isAdmin(userId) && adminOnlyButtons) {
            return safeAnswerCallbackQuery(bot, callbackQuery.id, {
        ...
        // callback already acknowledged at the beginning to avoid timeout/duplicate errors
...
// Additional low-level error handlers removed to avoid duplicate logging.
...
async function handlePayeerWithdrawalConfirm(chatId, messageId, data, language) {
    try {
        ...
        // Check balance again to prevent multiple or duplicate withdrawals
...
async function handleCashWithdrawalConfirm(chatId, messageId, data, language) {
    try {
        ...
        if (amount > currentBalance) {
            const message = language === 'en' ?
                `❌ Insufficient balance for this withdrawal.\n\n💰 Current balance: ${formatBalance(currentBalance, 'EGP')}\n💰 Requested: ${formatBalance(amount, 'EGP')}` :
...
async function handleWithdrawalCompletion(chatId, messageId, data, language) {
    try {
        ...
        // Get user info for notifications (balance was updated when request was created)
...
function parseEmailListText(text) {
    return Array.from(new Set(
        String(text || '')
            .split(/\r?\n|,|\s+/)
            .map(email => db.normalizeEmail(email))
            .filter(email => /^[^\s@]+@[^\s@]+\.[^\s@]+$/i.test(email))
    ));
}

async function processAddExportEmails(chatId, userId, text, language) {
    try {
        const emailList = parseEmailListText(text);
        if (emailList.length === 0) {
            return bot.sendMessage(
                chatId,
                language === 'en' ? '❌ No valid emails found. Send one email per line.' : '❌ لم يتم العثور على إيميلات صحيحة. أرسل كل إيميل في سطر.',
                keyboards.getKeyboard('cancelAdmin', language)
            );
        }

        const added = await db.addPersistentExportEmails(emailList);
        const total = (await db.getPersistentExportEmails()).length;
        userStates.delete(userId);

        return bot.sendMessage(
            chatId,
            language === 'en'
                ? `✅ Added ${added} email(s) to the persistent export list.
📦 Total saved: ${total}`
                : `✅ تم إضافة ${added} إيميل إلى قائمة التصدير الدائمة.
📦 العدد الحالي: ${total}`,
            keyboards.getKeyboard('bulkEmailManagement', language)
        );
    } catch (error) {
        console.error('Error adding export emails:', error);
        userStates.delete(userId);
        return bot.sendMessage(chatId, language === 'en' ? '❌ Error while adding emails.' : '❌ حدث خطأ أثناء إضافة الإيميلات.', keyboards.getKeyboard('bulkEmailManagement', language));
    }
}

async function processRemoveExportEmails(chatId, userId, text, language) {
    try {
        const emailList = parseEmailListText(text);
        if (emailList.length === 0) {
            return bot.sendMessage(
        ...
        const result = await db.removePersistentExportEmails(emailList);
        const total = (await db.getPersistentExportEmails()).length;
        userStates.delete(userId);
        ...
        return bot.sendMessage(
            chatId,
            language === 'en'
                ? `✅ ${result.hiddenFromExport} email(s) will now be hidden from export.
🗑 Removed from persistent list: ${result.removedFromPersistent}
📦 Remaining persistent emails: ${total}`
                : `✅ تم منع ${result.hiddenFromExport} إيميل من الظهور في التصدير.
🗑 تم حذف ${result.removedFromPersistent} من القائمة الثابتة.
📦 المتبقي في القائمة الثابتة: ${total}`,
            keyboards.getKeyboard('bulkEmailManagement', language)
        );
    } catch (error) {
        console.error('Error removing export emails:', error);
        userStates.delete(userId);
        return bot.sendMessage(chatId, language === 'en' ? '❌ Error while removing emails.' : '❌ حدث خطأ أثناء حذف الإيميلات.', keyboards.getKeyboard('bulkEmailManagement', language));
    }
}
...
async function exportAllEmails(chatId, language) {
    try {
        ...
        const persistentExportEmails = await db.getPersistentExportEmails();
        const excludedExportEmails = new Set(await db.getExportExcludedEmails());

        const visiblePendingEmails = pendingEmails.filter(item => !excludedExportEmails.has(db.normalizeEmail(item.email)));
        const visiblePendingGmails = pendingGmails.filter(item => !excludedExportEmails.has(db.normalizeEmail(item.email)));
        const visiblePersistentEmails = persistentExportEmails.filter(email => !excludedExportEmails.has(db.normalizeEmail(email)));

        const seenEmails = new Set();
        const dedupPendingEmails = visiblePendingEmails.filter(item => {
            const normalized = db.normalizeEmail(item.email);
            if (!normalized || seenEmails.has(normalized)) return false;
            seenEmails.add(normalized);
            return true;
        });
        const dedupPendingGmails = visiblePendingGmails.filter(item => {
            const normalized = db.normalizeEmail(item.email);
            if (!normalized || seenEmails.has(normalized)) return false;
            seenEmails.add(normalized);
            return true;
        });
        const dedupPersistentEmails = visiblePersistentEmails.filter(email => {
            const normalized = db.normalizeEmail(email);
            if (!normalized || seenEmails.has(normalized)) return false;
            seenEmails.add(normalized);
            return true;
        });

        if (dedupPendingEmails.length === 0 && dedupPendingGmails.length === 0 && dedupPersistentEmails.length === 0) {
            const message = language === 'en'
                ? '✅ No emails available for export right now.'
                : '✅ لا توجد إيميلات متاحة للتصدير الآن.';
            return bot.sendMessage(chatId, message);
        }

        let fileContent = language === 'en'
            ? `========================================
...
`;
        ...
        if (dedupPendingEmails.length > 0) {
...
`;
            ...
            dedupPendingEmails.forEach((email, index) => {
                fileContent += `${index + 1}. Email: ${email.email}
`;
                fileContent += `   Password: ${email.password}
`;
                if (email.user_id) fileContent += `   User ID: ${email.user_id}
`;
                if (email.created_at) fileContent += `   Created: ${email.created_at}
`;
                fileContent += `
`;
            });
...
`;
        ...
        if (dedupPendingGmails.length > 0) {
            fileContent += language === 'en'
...
`;
            ...
            dedupPendingGmails.forEach((gmail, index) => {
                fileContent += `${index + 1}. Email: ${gmail.email}
`;
                if (gmail.user_id) fileContent += `   User ID: ${gmail.user_id}
`;
                if (gmail.created_at) fileContent += `   Created: ${gmail.created_at}
`;
                fileContent += `
`;
            });
...
`;
        ...
        if (dedupPersistentEmails.length > 0) {
            fileContent += language === 'en'
                ? `📌 PERSISTENT EXPORT EMAILS:
----------------------------------------
`
                : `📌 الإيميلات الثابتة في التصدير:
----------------------------------------
`;

            dedupPersistentEmails.forEach((email, index) => {
                fileContent += `${index + 1}. Email: ${email}
...
`;
        ...
        const total = dedupPendingEmails.length + dedupPendingGmails.length + dedupPersistentEmails.length;
        fileContent += language === 'en'
            ? `========================================
SUMMARY:
========================================
Total Exported: ${total} emails
Regular Emails: ${dedupPendingEmails.length}
Gmail Accounts: ${dedupPendingGmails.length}
Persistent Export Emails: ${dedupPersistentEmails.length}
Hidden From Export: ${excludedExportEmails.size}

`
            : `========================================
الملخص:
========================================
إجمالي التصدير: ${total} إيميل
إيميلات عادية: ${dedupPendingEmails.length}
حسابات جيميل: ${dedupPendingGmails.length}
إيميلات ثابتة: ${dedupPersistentEmails.length}
مخفية من التصدير: ${excludedExportEmails.size}
...
📊 Total: ${total} emails
⏳ Regular: ${dedupPendingEmails.length}
📱 Gmail: ${dedupPendingGmails.length}
📌 Persistent: ${dedupPersistentEmails.length}
🙈 Hidden: ${excludedExportEmails.size}`
            : `📤 تصدير الإيميلات المعلقة

📊 الإجمالي: ${total} إيميل
⏳ عادية: ${dedupPendingEmails.length}
📱 جيميل: ${dedupPendingGmails.length}
📌 ثابتة: ${dedupPersistentEmails.length}
🙈 مخفية: ${excludedExportEmails.size}`;

        await bot.sendDocument(chatId, filename, { caption });
        fs.unlinkSync(filename);
        console.log(`Exported ${total} emails to ${filename}`);
    } catch (error) {
        console.error('Error exporting emails:', error);
        const errorMessage = language === 'en' ? '❌ Error exporting emails' : '❌ حدث خطأ في تصدير الإيميلات';
        bot.sendMessage(chatId, errorMessage);
    }
}
...
async function processSelectiveApproval(chatId, userId, text, language) {
    try {
        ...
        // Store IDs in memory to avoid Telegram's 64-byte callback_data limit
        const actionKey = `sa_${chatId}_${Date.now()}`;
        pendingBulkActions.set(actionKey, {
            gmailIds: matchedGmails.map(g => g.id),
            emailIds: matchedEmails.map(e => e.id),
            timestamp: Date.now()
        });
        // Auto-cleanup after 10 minutes
        setTimeout(() => pendingBulkActions.delete(actionKey), 10 * 60 * 1000);
                    ...
                    [
                        {
                            ...
                            callback_data: `selective_approve_confirm_${actionKey}`
                        },
                        {
                            text: language === 'en' ? '❌ Cancel' : '❌ إلغاء',
...
async function processSelectiveRejection(chatId, userId, text, language) {
    try {
        ...
        // Store IDs in memory to avoid Telegram's 64-byte callback_data limit
        const actionKey = `sr_${chatId}_${Date.now()}`;
        pendingBulkActions.set(actionKey, {
            gmailIds: matchedGmails.map(g => g.id),
            emailIds: matchedEmails.map(e => e.id),
            timestamp: Date.now()
                    ...
                    [
                        {
                            ...
                            callback_data: `selective_reject_confirm_${actionKey}`
                        },
                        {
                            text: language === 'en' ? '🔙 Cancel' : '🔙 إلغاء',
            ...
            try {
                ...
                if (user) {
                    ...
                    const displayReward = currency === 'USD'
                        ? `$${(await convertEGPToUSD(gmailReward)).toFixed(3)}`
                        : formatBalance(gmailReward, 'EGP');
                    const approvalMessage = formatTaskDecisionMessage({
                        type: 'gmail',
                        status: 'approved',
                        email: gmail.email,
...
async function processSelectiveApprovalConfirm(chatId, messageId, data, language) {
    try {
        // Support both new key-based format and legacy ID-based format
        let gmailIds = [];
        let emailIds = [];

        if (data.startsWith('selective_approve_confirm_')) {
            // New format: look up IDs from pendingBulkActions map
            const actionKey = data.replace('selective_approve_confirm_', '');
            const stored = pendingBulkActions.get(actionKey);
            if (!stored) {
                const expiredMsg = language === 'en' ?
                    '❌ Action expired. Please try again.' :
                    '❌ انتهت صلاحية الإجراء. حاول مرة أخرى.';
                return bot.editMessageText(expiredMsg, { chat_id: chatId, message_id: messageId });
            }
            gmailIds = stored.gmailIds;
            emailIds = stored.emailIds;
            pendingBulkActions.delete(actionKey);
        } else {
            // Legacy format: parse IDs from callback data
            const parts = data.replace('selective_approve_', '').split('_');
            gmailIds = parts[0] ? parts[0].split(',').filter(id => id).map(id => parseInt(id)) : [];
            emailIds = parts[1] ? parts[1].split(',').filter(id => id).map(id => parseInt(id)) : [];
        }
        ...
        for (const emailId of emailIds) {
            try {
                ...
                if (user) {
                    ...
                    const displayReward = currency === 'USD'
                        ? `$${(await convertEGPToUSD(emailReward)).toFixed(3)}`
                        : formatBalance(emailReward, 'EGP');
                    const approvalMessage = formatTaskDecisionMessage({
                        type: 'email',
                        status: 'approved',
                        email: email.email,
...
async function processSelectiveRejectionConfirm(chatId, messageId, data, language) {
    try {
        ...
        if (data.startsWith('selective_reject_confirm_')) {
            // New format: look up IDs from pendingBulkActions map
            const actionKey = data.replace('selective_reject_confirm_', '');
            const stored = pendingBulkActions.get(actionKey);
            if (!stored) {
                const expiredMsg = language === 'en' ?
                    '❌ Action expired. Please try again.' :
        ...
        for (const gmailId of gmailIds) {
            try {
                ...
                const userLanguage = await getUserLanguage(gmail.user_id);
                const rejectionMessage = formatTaskDecisionMessage({
                    type: 'gmail',
                    status: 'rejected',
                    email: gmail.email,
                    language: userLanguage
...
async function startBotSafely() {
    try {
        console.log(`🚀 Starting Telegram Bot${config.TERMUX_MODE ? ' (Termux mode)' : ''}...`);
        ...
        console.log(`📱 Runtime: ${config.TERMUX_MODE ? 'Termux / Android' : 'Standard Node.js'}`);
        ...
        if (error.message.includes('401')) {
            console.error('🔑 Invalid bot token. Please check your BOT_TOKEN in .env');
